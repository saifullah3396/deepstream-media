...
static MediaAppCtx *mediaAppCtx;
                            ...
                            gchar *src_uri, gint stream_id)
{
    ...
    if (playback_utc || (appCtx[0]->config.multi_source_config[stream_id].type !=
                         NV_DS_SOURCE_RTSP))
    {
        if (mediaAppCtx->streams[stream_id].meta_number == 0)
        {
            mediaAppCtx->streams[stream_id].timespec_first_frame =
                extract_utc_from_uri(src_uri);
            memcpy(&tloc,
                   (void *)(&mediaAppCtx->streams[stream_id].timespec_first_frame.tv_sec), sizeof(time_t));
            ms = mediaAppCtx->streams[stream_id].timespec_first_frame.tv_nsec /
                 1000000;
            mediaAppCtx->streams[stream_id].gst_ts_first_frame = ts;
            ts_generated =
                GST_TIMESPEC_TO_TIME(mediaAppCtx->streams[stream_id].timespec_first_frame);
            if (ts_generated == 0)
            {
        ...
        else
        {
            GstClockTime ts_current =
                GST_TIMESPEC_TO_TIME(mediaAppCtx->streams[stream_id].timespec_first_frame) + (ts -
                                                                                              mediaAppCtx->streams[stream_id].gst_ts_first_frame);
                                     ...
                                     NvDsBatchMeta *batch_meta, guint index)
{
         ...
         l_frame = l_frame->next)
    {
       ...
       */
            StreamSourceInfo *src_stream = &mediaAppCtx->streams[stream_id];
       ...
       * be displayed on top of the bounding box, so lets form it here. */
         ...
         */
           ...
           */
                ...
                mediaAppCtx->streams[stream_id].meta_number++;
        ...
        mediaAppCtx->streams[stream_id].frameCount++;
    }
}
...
{
    mediaAppCtx = (MediaAppCtx *)g_malloc0(sizeof(MediaAppCtx));
    ...
    for (i = 0; i < num_instances; i++)
    {
        ...
        if (override_cfg_file && override_cfg_file[i])
        {
            if (!g_file_test(override_cfg_file[i],
                             static_cast<GFileTest>(G_FILE_TEST_IS_REGULAR | G_FILE_TEST_IS_SYMLINK)))
            {
