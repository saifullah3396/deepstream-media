...
typedef enum NvDsObjectType {
  ...
  NVDS_OBJECT_TYPE_SD_VIOLATION = 0x103,
...
typedef enum NvDsPayloadType {
  ...
  NVDS_PAYLOAD_SD_VIOLATION = 0x102,
...
/**
 * Holds a social distancing violation object's parameters.
 */
typedef struct NvDsSDViolationObject {
  gint violation_cluster_id; /**< Holds a pointer to violation cluster the object belongs to. */
} NvDsSDViolationObject;
