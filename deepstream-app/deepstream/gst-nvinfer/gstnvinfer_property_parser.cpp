    ...
    const gchar * cfg_file_path)
{
  ...
  if (!g_strcmp0 (key, CONFIG_GROUP_INFER_PROCESS_MODE)) {
    ...
    CHECK_ERROR (error);
  } else if (!g_strcmp0 (key, CONFIG_GROUP_INFER_SCALE_WITH_HEIGHT)) {
    if (g_key_file_get_boolean (key_file, group_name,
            CONFIG_GROUP_INFER_SCALE_WITH_HEIGHT, &error))
      nvinfer->scale_with_height = TRUE;
    CHECK_ERROR (error);
    ...
    GKeyFile * key_file, const gchar * cfg_file_path)
{
  ...
  for (key = keys; *key; key++) {
    ...
    CHECK_ERROR (error);
      ...
      CHECK_ERROR (error);
    } else if (!g_strcmp0 (*key, CONFIG_GROUP_INFER_DYNAMIC_INPUT_SIZE)) {
      init_params->dynamicInputSize =
          g_key_file_get_boolean (key_file, CONFIG_GROUP_PROPERTY,
          CONFIG_GROUP_INFER_DYNAMIC_INPUT_SIZE, &error);
      CHECK_ERROR (error);
